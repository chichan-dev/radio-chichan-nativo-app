...
public class Macroblock {
    ...
    public Macroblock(int y, int x) {
        ...
        for (int row = 0; row < 4; row++)
            for (int col = 0; col < 4; col++)
                this.ySubblocks[row][col] = new Subblock(row, col, PLANE.Y1);
        for (int row = 0; row < 2; row++)
            for (int col = 0; col < 2; col++) {
                uSubblocks[row][col] = new Subblock(row, col, PLANE.U);
                vSubblocks[row][col] = new Subblock(row, col, PLANE.V);
            }
    }
    ...
    public Subblock getBottomSubblock(int x, PLANE plane) {
        if (plane == PLANE.Y1) {
            return ySubblocks[3][x];
        } else if (plane == PLANE.U) {
            return uSubblocks[1][x];
        } else if (plane == PLANE.V) {
            return vSubblocks[1][x];
        } else if (plane == PLANE.Y2) {
            return y2;
        }
        return null;
    }

    public Subblock getRightSubBlock(int y, PLANE plane) {
        if (plane == PLANE.Y1) {
    ...
    private void decodeMacroBlockTokens(boolean withY2, Macroblock[][] mbs, BooleanArithmeticDecoder decoder, int[][][][] coefProbs) {
        skipFilter = false;
        if (withY2) {
            skipFilter = skipFilter | decodePlaneTokens(1, PLANE.Y2, false, mbs, decoder, coefProbs);
        }
        skipFilter = skipFilter | decodePlaneTokens(4, PLANE.Y1, withY2, mbs, decoder, coefProbs);
        skipFilter = skipFilter | decodePlaneTokens(2, PLANE.U, false, mbs, decoder, coefProbs);
        skipFilter = skipFilter | decodePlaneTokens(2, PLANE.V, false, mbs, decoder, coefProbs);
        skipFilter = !skipFilter;
    }

    private boolean decodePlaneTokens(int dimentions, PLANE plane, boolean withY2, Macroblock[][] mbs, BooleanArithmeticDecoder decoder, int[][][][] coefProbs) {
        boolean r = false;
        for (int row = 0; row < dimentions; row++) {
            for (int col = 0; col < dimentions; col++) {
                int lc = 0;
                Subblock sb = null; //this.ySubblocks[row][col];
                if (PLANE.Y1.equals(plane)) {
                    sb = ySubblocks[row][col];
                } else if (PLANE.U.equals(plane)) {
                    sb = uSubblocks[row][col];
                } else if (PLANE.V.equals(plane)) {
                    sb = vSubblocks[row][col];
                } else if (PLANE.Y2.equals(plane)) {
                    sb = y2;
                }
    ...
    public class Subblock {
        ...
        public Subblock(int row, int col, PLANE plane) {
            this.row = row;
            this.col = col;
            this.plane = plane;
        }
        ...
        public Subblock getLeft(PLANE p, Macroblock[][] mbs) {
            if (this.col > 0)
                if (PLANE.Y1.equals(this.plane))
